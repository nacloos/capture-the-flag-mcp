class GameMap {
    ...
    findNearestClearPosition(x, y, size, maxDistance = 100) {
        // Find nearest position that doesn't collide
        let bestPos = null;
        let bestDistance = Infinity;
        
        for (let angle = 0; angle < TWO_PI; angle += PI/8) {
            for (let distance = size * 2; distance <= maxDistance; distance += size) {
                let testX = x + cos(angle) * distance;
                let testY = y + sin(angle) * distance;
                
                if (!this.checkCollision(testX, testY, size)) {
                    let actualDistance = dist(x, y, testX, testY);
    ...
    hasMultiplePaths(startX, startY, endX, endY, size) {
        // Simple check to see if there are multiple navigation paths
        // This helps ensure the map has good connectivity
        ...
        for (let i = 0; i <= steps; i++) {
            ...
            if (this.checkCollision(x, y, size)) {
                directBlocked = true;
                break;
            }
        }
        
        if (!directBlocked) return true; // Direct path available
        
        // Check if alternative paths exist by testing detour routes
        let alternativeRoutes = [
            {midX: startX, midY: endY}, // L-shaped path
            {midX: endX, midY: startY}, // L-shaped path (other direction)
            {midX: (startX + endX) / 2, midY: startY - 200}, // Northern detour
            {midX: (startX + endX) / 2, midY: startY + 200}  // Southern detour
        ];
        ...
        for (let route of alternativeRoutes) {
            ...
            for (let i = 0; i <= steps; i++) {
                let t = i / steps;
                let x = lerp(startX, route.midX, t);
                let y = lerp(startY, route.midY, t);
            ...
            if (path1Clear) {
                for (let i = 0; i <= steps; i++) {
                    let t = i / steps;
                    let x = lerp(route.midX, endX, t);
                    let y = lerp(route.midY, endY, t);
